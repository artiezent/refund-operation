...
function syncPipedriveData() {
  ...
  // Won deals → 결제완료 시트 (2025년+ 성사 OR 결제추적 필드 있는 거래)
  var wonDeals = fetchDealsByStatus('won');
  var wonFiltered = [];
  for (var i = 0; i < wonDeals.length; i++) {
    var wt = wonDeals[i].won_time || '';
    var wy = parseInt(String(wt).substring(0, 4)) || 0;
    if (wy >= 2025 || isRelevantDeal(wonDeals[i])) {
      wonFiltered.push(wonDeals[i]);
    }
  }
  var wonSheet = getOrCreateSheet('결제완료');
  writeDealsToSheet(wonSheet, wonFiltered, stageMap);
  Logger.log('결제완료: ' + wonFiltered.length + '건 (전체 Won: ' + wonDeals.length + '건)');
...
function syncBatchUpdate() {
  ...
  for (var k = 0; k < prevKeys.length; k++) {
    ...
    if (deal && deal.won_time && (parseInt(String(deal.won_time).substring(0,4)) >= 2025 || isRelevantDeal(deal))) {
